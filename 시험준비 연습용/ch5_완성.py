...
print(subway)
print(subway.count("푸")) # subway 안에 "푸"가 몇 개 있는지 확인
...
num_list.sort(reverse=True) # 내림차순 정렬렬
print(num_list)

# (형식) 리스트.reverse()
# - reverse() 함수는 정렬과는 전혀 상관없음
# - 단순히 리스트 안에 있는 값을 순서를 뒤집는 역할만 함
...
print(3 in cabinet) # key 3이 있기 때문에 True 출력
print(5 in cabinet) # key 5가 없기 때문에 False 출력
...
# 튜플의 값의 접근 : 인덱스를 이용
menu = ("돈까스", "치즈돈까스")
print(menu[0]) # 0번 인덱스의 값 선택
print(menu[1]) # 1번 인덱스의 값 선택
...
#tup1[2] = 40 # 읽기 전용이기 때문에 값을 변경하지 못함

# 값을 변경해야 한다면???
# - 리스트 타입으로 전환 후 값을 변경하고 다시 튜플 타입으로 전환환
list1 = list(tup1)
list1[2] = 40
tup1 = tuple(list1)
print(tup1)
...
sef_list = ["자료구조", "알고리즘", "자료구조", "운영체제"]
sef_set = set(sef_list)
sef_list = list(sef_set)
