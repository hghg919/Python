...
idx = python.index("n", idx + 1) # 인덱스 6 이후부터 찾아 처음 발견한 n의 인덱스 검색
print(idx)
idx = python.index("n", 2, 6) # 인덱스 2부터 6 직전까지 찾아 처음 발견한 n의 인덱스 검색
print(idx) # "thon"에서 n (인덱스 5)
# idx = python.index("Java") # "Java"가 없으면 -1을 반환(출력) 한 후 프로그램 계속 수행
# print(idx)

print(python.count("n")) # "n"이 문자열 안에서 몇 번 나오는지 확인
print(python.count("v")) # "v"이 문자열 안에서 몇 번 나오는지 확인

print(len(python)) # python 변수에 저장된 문자열의 길이 반환

########################################################
# 4.4. 문자열 포맷팅(String Formatting)
...
# - 자료형을 표현하는 방법
# - % 뒤에 자료형을 나타내는 문자 표기
print("나는 %d살입니다." % 20) # %d : 정수(decimal)
print("나는 %s(을)를 좋아합니다." % "파이썬") # %s : 문자열(string)
...
# - 문자열에서 값을 넣을 위치에 중괄호({})를 표시하고 뒤에 format(값1, 값2, ...) 형태로 값을 입력
# - 인덱스를 생략하고 {}만 적으면 format() 함수 안 값들이 {} 위치에 순서대로 들어감
...
# 4.5.3. 역슬래시 (\\)
# print("C:\Users\D503\Desktop\PythonWorkspace")
print("C:\\Users\\D503\\Desktop\\PythonWorkspace")
print(r"C:\Users\D503\Desktop\PythonWorkspace") # 문자열 내에 어떤 값이 있든지 무시하고 그대로 출력
